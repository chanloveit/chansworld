...
이걸 [ANSI Escape Code](https://namu.wiki/w/ANSI%20%EC%9D%B4%EC%8A%A4%EC%BC%80%EC%9D%B4%ED%94%84%20%EC%BD%94%EB%93%9C)라는 걸로 지워 버렸다.  
```ts
if(currentChannel){
			try{
...
```
\u001b[1A <- 뒤의 nA는 n만큼 커서를 위로 올리는 동작, \u001b[2K는 전체 줄을 지우는 동작이다.  
 뭔가 옛스러운 느낌이 들어 좋다. 
